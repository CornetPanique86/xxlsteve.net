...
fixedsearch = function(){
	var searchPageblock = document.getElementById("search-pageblock");
	...
	// Display search when clicking on the nav button
	...
	function search_toggle_focus(e) {
		...
		if (!search__focus) {
			...
			openPageBlock(searchPageblock.id);
			...
			closePageBlock(searchPageblock.id);
			...
			for (let item in results.slice(0,5)) { // only show first 5 results
				search_items = search_items +
					`<li><a href="${results[item].item.permalink}" tabindex="0">
						<div class="s-date"><span>${results[item].item.date}</span></div>
						<div class="s-title">
							<span>${results[item].item.section}</span>
